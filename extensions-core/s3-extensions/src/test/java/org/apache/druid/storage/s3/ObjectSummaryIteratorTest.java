...
public class ObjectSummaryIteratorTest
{
  ...
  )
  {
    ...
    for (final String uri : expectedUris) {
      final List<S3ObjectSummary> matches = TEST_OBJECTS.stream()
                                                        .filter(
                                                            summary ->
                                                                summaryToUri(summary).toString().equals(uri)
                                                        )
    ...
    Assert.assertEquals(
        prefixes.toString(),
        expectedObjects.stream().map(ObjectSummaryIteratorTest::summaryToUri).collect(Collectors.toList()),
        actualObjects.stream().map(ObjectSummaryIteratorTest::summaryToUri).collect(Collectors.toList())
    );
  }
  ...
  private static URI summaryToUri(final S3ObjectSummary object)
  {
    ...
    return URI.create(
        StringUtils.format(
            ...
            StringUtils.replace(StringUtils.urlEncode(object.getKey()), "%2F", "/")
        )
    );
  }
}
