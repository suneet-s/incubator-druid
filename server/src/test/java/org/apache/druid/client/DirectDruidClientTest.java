...
public class DirectDruidClientTest
{
  ...
  public void testRun() throws Exception
  {
    ...
    Sequence s1 = client.run(QueryPlus.wrap(query), defaultContext);
    ...
    client.run(QueryPlus.wrap(query), defaultContext);
  ...
  public void testCancel()
  {
    ...
    Sequence results = client.run(QueryPlus.wrap(query), defaultContext);
  ...
  public void testQueryTimeoutBeforeFuture() throws IOException, InterruptedException
  {
    ...
    context.put(DirectDruidClient.QUERY_FAIL_TIME, System.currentTimeMillis() + 250);
    context.put("queryId", queryId);
    context.put(DirectDruidClient.QUERY_TOTAL_BYTES_GATHERED, new AtomicLong());

    TimeBoundaryQuery query = Druids.newTimeBoundaryQueryBuilder().dataSource("test").context(context).build();
    Sequence results = client.run(QueryPlus.wrap(query), context);
  ...
  public void testQueryTimeoutFromFuture()
  {
    ...
    Map<String, Object> context = new HashMap<>();
    context.put(DirectDruidClient.QUERY_FAIL_TIME, System.currentTimeMillis() + 500);
    context.put("queryId", queryId);
    context.put(DirectDruidClient.QUERY_TOTAL_BYTES_GATHERED, new AtomicLong());
